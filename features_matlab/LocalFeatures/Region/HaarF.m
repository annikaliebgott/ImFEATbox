...
% length
    ...
    if(length == 1)
        % feature determined only for one pixle
        ...
        temp_1 = I((x-b2):(x+b2),(y-b2):(y));
        temp_2 = I((x-b2):(x+b2),(y):(y+b2));
        two_rect_vert = sum(temp_1(:)) - sum(temp_2(:));

    else
        % case when the feature should determined for more than one pixle
