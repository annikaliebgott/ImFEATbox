...
for n=1:1:size(GLCM_Matrices,3)
    ...
    if SumOfGLCM > 0
        ...
        % Entropy (H)
        if typeflag.entropy
            % avoid NaN-Values in H by substituting zero values in GNormalized
            % with very small value eps
            GNormalized_H = GNormalized;
            GNormalized_H(GNormalized_H == 0) = eps;
            H = - sum(GNormalized(:).*log(GNormalized_H(:)));
        end
        ...
        % Summed entropy (SE)
        if typeflag.entropy
            % avoid NaN-Values in SE by substituting zero values in p_xplusy
            % with very small value eps
            p_xplusy_SE = p_xplusy;
            p_xplusy_SE(p_xplusy_SE == 0) = eps;
            SE = -sum(p_xplusy.*log(p_xplusy_SE));
        end
        ...
        if typeflag.entropy
            % avoid NaN-Values in DE by substituting zero values in p_xminusy
            % with very small value eps
            p_xminusy_DE = p_xminusy;
            p_xminusy_DE(p_xminusy_DE == 0) = eps;
            DE = -sum(p_xminusy.*log(p_xminusy_DE));
            
            HXY = H;
            
            for i = 1:s1
                % avoid NaN-Values in HXY1 and HXY2 by substituting zero values
                % in p_x and p_y with very small value eps
                p_x_H = p_x;
                p_y_H = p_y;
                p_x_H(p_x_H == 0) = eps;
                p_y_H(p_y_H == 0) = eps;
                HXY1 = HXY1 - GNormalized(i,:)*log(p_x_H(i).*p_y_H);
                HXY2 = HXY2 - sum(p_x(i).*p_y.*log(p_x_H(i).*p_y_H));
            end
            
            Hx = - sum(p_x.*log(p_x_H));
            Hy = - sum(p_y.*log(p_y_H));
