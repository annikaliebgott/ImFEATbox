...
%   if it already exists updates the value. LABEL must be a string or a 
%   cell array: label{1} = updated Figure title label{2} = labeltext and
...
% check for figure name update
if(iscell(label))
    set(figh,'Name',label{1})
    % for consistency
    label = label{2};
end
